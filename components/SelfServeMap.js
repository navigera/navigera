...
export default class SelfServeMap extends Component {
  ...
  changePosition(aisle, shelf){
      ...
      if(pos !== undefined)
        this.setState({plRow: pos.row, plCol:pos.col});
  }
  
  //If change in carousel causes re-rendering of imagebackground, recalculate everything
  mutateImage(width, height){
      this.setState({imageHeight: height, imageWidth: width, rowSize: height/GRIDHEIGHT, colSize: width/GRIDWIDTH});  
  }
  ...
  render() {
    return (
        ...
        <ImageBackground
          onLayout ={
            event => {
                this.mutateImage(event.nativeEvent.layout.width, event.nativeEvent.layout.height);
            }
          }
          source={{uri: 'http://alis.pm/test.png'}} 
          style = {styles.backgroundImage}
          resizeMethod = 'scale'
          resizeMode = 'stretch'>
            <PackageLocation onPress = {
                () => {
                    this.changePosition(1,29)
                }
            } row = {this.state.plRow} col = {this.state.plCol} width = {this.state.colSize} height = {this.state.rowSize}/>
        </ImageBackground>
    );
  }
}
...
class PackageLocation extends Component{
  ...
  render(){
    ...
    return (
      ...
      style= {{width: this.props.width*2, height:this.props.height, backgroundColor: 'yellow', top: rowpos, left:colpos}}/>
    )
}
}
